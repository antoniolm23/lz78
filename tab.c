...
int hash(int father, int size) {
	...
	return (father % size);	/* Delimit the result to the size of the table*/
}
...
int hash_add(int index, int father, unsigned int symbol, ht_table* table) {
	...
	if(table->next_label == ((table->total_size/4)*3))	/* Hash table full? */
		return -1;
...
int tab_insertion(int father, unsigned int symbol, table* t) {
	...
	if(t->next_pos == ((t->size/4)*3))	/* Full table? */
		return -1;
	else
		return pos;
}
