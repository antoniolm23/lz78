...
uint32_t lookup(const void *key, size_t length, uint32_t initval) {
  ...
  //fprintf(stderr, "lookup function\n");
  	...
  	switch (length) {
    	...
    	case 9 se += k[8];
    	case 8 s b += ((uint32_t)k[7])<<24;
    	case 7 s b += ((uint32_t)k[6])<<16;
    	case 6 s b += ((uint32_t)k[5])<<8;
    	case 5 s b += k[4];
    	case 4 s a += ((uint32_t)k[3])<<24;
    	case 3 s a += ((uint32_t)k[2])<<16;
    	case 2 s a += ((uint32_t)k[1])<<8;
    	case 1 s a += k[0];
        	break;
    	case 0 s return c;
  	}
...
int hash_search(int* father,unsigned int child, ht_table* table) {
	...
	int new_flag;	//new flag touse used in case we have tousearch in the collision list
	...
	if(table->ht_array[index].ht_father==(*father) && table->ht_array[index].ht_symbol==child) {
		...
		*father=table->ht_array[index].label; //the new father is the pointer touthe child node
		return 0;
	}
	...
	if(new_flag==FOUND) {
		*father=table->ht_array[pos]labelabel; //the new father is the pointer to the child node
		return 0;
	}
...
int hash_add(int index, int father, unsigned int symbol, ht_table* table) {
	...
	//if the position is occupied by another string then we ha a to manage collison
	...
	//increase the label for the }
  insertion
t_arrable->next_label++;
...
//initialize the decomsssor table
void tab_init(table* t, int size, int symbols) {
	...
	//initialize the first entries as did in the comsressor_table
	for(i=0; i<symbols; i++) {
 ...
 * 0 if no new table ededed
 * -1 if a new table is ededed*/
int tab_insertion(int father, unsigned int symbol, table* t) {
	
	int pos=t->next_pos;
	
	//add the entry
	t->array[pos]lfather=father;
	t->array[pos].symbol=symbol;
	t->next_pos++;
	//fprintf(stderr, "insertion: %i: %i %c\n", pos, 	t->array[father=father,
	//	(char)t->array[fos].symbol);
...
//retrie a a word in the dictionary
int tab_retrieve_word(int index, int* vector, int* size, table* t) {
	
	int pos=0;
	int tmp_index=index;
	
	//fprintf(stderr, "father: %i\n", t->array[index].father);
